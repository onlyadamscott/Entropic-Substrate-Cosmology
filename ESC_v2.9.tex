...
\textbf{Information--Mass Equivalence (effective contribution):}
\begin{equation}
m_{\rm info} = \frac{k_B s}{c^2} \tag{10}
\end{equation}
...
\textbf{QM:} The free-evolution map \(F\) acts on the projective Hilbert space of normalized states. The Hilbert space structure and unitary evolution emerge naturally because the entropy substrate preserves the norm of \(\Psi\) between context locks, yielding standard Schrödinger/Dirac evolution in the continuum limit. The context-lock operator \(\hat{C}\) provides thermodynamic projection. The Born rule \(f_k = |\langle o_k | \Psi \rangle|^2\) emerges variationally as the minimizer of the KL divergence \(\operatorname{KL}(p \| f)\) in the average entropy-production rate, consistent with the monotonicity of quantum relative entropy under projective measurements.

\textbf{EM:} The quantum-like state \(\Psi\) is complex-valued. Local U(1) phase redundancy \(\Psi \to e^{i\alpha(x)} \Psi\) is required to preserve the norm and probability interpretation under the entropy substrate. This local symmetry necessitates the introduction of the gauge connection \(A_\mu\) to make the covariant derivative phase-invariant, yielding the field strength \(F_{\mu\nu} = \partial_\mu A_\nu - \partial_\nu A_\mu\). The minimal gauge-invariant term \(-\frac{1}{4} F_{\mu\nu} F^{\mu\nu}\) then yields Maxwell’s equations sourced by entropy/information currents.
...
\subsection{8.1 Potential Resolution of the Hubble Tension}
A promising cosmological implication of EWC is that high entropy-production rates in the early universe could naturally contribute a transient effective energy density \(\rho_{\rm irr}\) that modifies the expansion history.

The modified Friedmann equation in a flat FLRW universe becomes
\[
H^2 = \frac{8\pi G}{3} \Bigl( \rho + \rho_{\rm irr} \Bigr) + \frac{\Lambda c^2}{3},
\]
where
\[
\rho_{\rm irr}(a) = \rho_{\rm irr,0} \left( \frac{a_{\rm ref}}{a} \right)^{4 + \delta}, \qquad \delta > 0.
\]

This form behaves like extra radiation at early times and decays faster than radiation after a reference scale factor \(a_{\rm ref}\). A suitably chosen early-universe entropy-production spike can increase the early expansion rate, reducing the sound horizon at recombination and raising the inferred \(H_0\) from CMB data toward the higher local value, while decaying sufficiently rapidly to remain compatible with BBN and late-time observables (BAO, weak lensing).

Quantitative fitting with Boltzmann codes (CLASS/CAMB) would be required to determine whether realistic \(\rho_{\rm irr}(a)\) profiles can reduce the current \(\sim 5\sigma\) tension to \(<2\sigma\) while preserving the success of CMB acoustic peaks and BBN. This remains a promising direction for future work.

\subsection{8.2 Laboratory Analog: False Vacuum Decay}
Recent experiments using rings of Rydberg atoms have successfully simulated false vacuum decay — a quantum tunneling process in which a metastable “false vacuum” state decays into a lower-energy “true vacuum” state, forming expanding bubbles. This process mirrors a cosmic-scale context lock: a high-entropy metastable superposition collapses into a definite lower-energy vacuum, with the discarded possibilities irreversibly absorbed into the entropy substrate. The expanding bubble wall corresponds to a propagating front of extremely high entropy production \(\sigma\), providing a concrete laboratory analog for the entropy-driven phase transitions EWC predicts in the early universe. Such analogs strengthen the framework’s claim that vacuum transitions are fundamentally thermodynamic events governed by the substrate.
...
\section{Appendix C: Black Hole Entropy and the Area Law}
The Bekenstein-Hawking area law is recovered naturally in EWC. On a stationary Killing horizon generated by \(\xi^\mu\), the entropy current must satisfy the second-law constraint \(\nabla_\mu J^\mu \geq 0\) and the Killing symmetry. This forces \(J^\mu \parallel \xi^\mu\) on the horizon. The total entropy is the flux through a spacelike cross-section \(\Sigma\):
\[
S = \int_{\Sigma} J^\mu \, d\Sigma_\mu.
\]
Matching to the surface gravity definition of temperature \(T = \kappa / 2\pi\) and the first law recovers the classical result
\[
S = \frac{k c^3 A}{4 \hbar G}.
\]

The wastefield term introduces a small logarithmic correction
\[
\delta S \approx \beta \ln \left( \frac{A}{A_0} \right),
\]
which encodes memory of past context locks and provides a geometric mechanism for information preservation without violating the second law.
